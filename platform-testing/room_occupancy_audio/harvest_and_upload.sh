...
if  [[ $label != "-n"  &&  $label != "-p" ]]
   then
   echo "Error: must provide -e (empty) or -a (activity) as first argument to label the data being sampled." >&2; exit 1
fi
...
do
   python ./audio_collection.py $label -l $length_of_sample
done

# echo "Uploading csv files..."
edge-impulse-uploader ./samples/*.wav